...
**The "done" bar after a release (non-negotiable):** a build is done only when the journey suite is GREEN against the **freshly-deployed production URL** — run it *after* `vercel --prod`, not against localhost and not against a possibly-stale `.next` (`rm -rf .next` before a release build if unsure). A debug stub once passed locally on a stale bundle and broke every answer in prod: `../gotchas/committed-debug-stub-broke-prod.md`.
...
| `case-file-qa.spec.ts` | The case-file-qa feature: F-A Final Judgment ($1,285 + Joni Carter, `[P:family-court#24]`), F-B grounds corroborated across **both** PDFs (court file + story), F-C the filing-date **conflict surfaced** (Feb 10 vs Feb 3), F-D the Final Judgment in Hebrew. Asserts page-level citations, multi-document corroboration, conflict-surfacing, and the `validateCaseAnswer()` "Grounded" pass. |
| `maintenance-spend-intelligence.spec.ts` | The maintenance-spend feature: F-A the marquee **honest refusal** of the overdue/suspension question (schema-cited, pivots to $40,597 spend, **no fabricated overdue list**, no Carter leak), F-B a real spend answer ($40,597 total / 2026 $13,485.66/248 / Oyoba top, cited rows), F-C the refusal in Hebrew. Asserts `validateNoFabrication()` and routing to maintenance (not the unrelated documents). |
